...
def plotAccuracyDistribution(df: pd.DataFrame) -> None:
    ...
    ax.legend(loc='best')
    plt.subplots_adjust(bottom=0.1, top=0.95, left=0.06, right=0.95)
...
def plotRoundScores(scores: dict, players: list = None, colors: list = None, title: str = None):
    """
    ...
    for player, score in scores.items():
        ...
        ax.plot(range(len(score)), pScore, label=player, alpha=1, linestyle=linestyles[index%len(linestyles)], color=colors[index%len(colors)], linewidth=3)
        index += 1
    
    fig.subplots_adjust(bottom=0.1, top=0.95, left=0.06, right=0.95)
    ax.legend()
...
def plotScoresArmageddon(scores: dict, filename: str = None) -> None:
    ...
    ax.set_facecolor('#e6f7f2')
    # ax.legend()
    fig.subplots_adjust(bottom=0.2, top=0.95, left=0.06, right=0.95)
...
if __name__ == '__main__':
    ...
    for p, v in gameAcc.items():
        ...
        avgGameAcc[p] = [pA, oA]
    # generateTournamentPlots(wijk, nicknames=nicknames)
    # plotBarChart(avgGameAcc, ['Player accuracy', 'Opponent accuracy'], 'Game accuracy', 'Game accuracy', short=nicknames)
    ...
    plotRoundScores(roundScores, players=fightForFirst, title='Fight for 1st place')
    # Ding games
    """
