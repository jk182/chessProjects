    ...
    with open(pgnPath, 'r') as pgn:
        while game := chess.pgn.read_game(pgn):
            ...
            if len(wxs) != 0 and len(bxs) != 0:
                ...
                print(f'No game accuracies: {w}-{b}')
    return accuracies
...
def getOpenings(pgnPath: str, shortNames: bool = False) -> dict:
    ...
    with open(pgnPath, 'r') as pgn:
        while game := chess.pgn.read_game(pgn):
            if "Opening" not in game.headers or len(list(game.mainline_moves())) == 0:
                continue
...
def plotDecisiveGames(decisiveGames: dict, nicknames: dict = None, title: str = 'Relative number of wins and losses', relativeNumbers: bool = True, filename: str = None):
    """
    ...
    relativeNumbers: bool
        If this is true, all numbers will be relative to the total numbers
    filename: str
        The name of the file to which the graph should be saved.
        If no name is specified, the graph will be shown instead of saved.
    """
    ...
    if relativeNumbers:
        decisiveGames = dict(reversed(sorted(decisiveGames.items(), key=lambda item: (item[1][0]+item[1][1])/item[1][2])))
    else:
        decisiveGames = dict(reversed(sorted(decisiveGames.items(), key=lambda item: item[1][0]+item[1][1])))
...
def generateTournamentPlots(pgnPath: str, nicknames: dict = None, players: list = None, lichessAnalysis: bool = False, firstPlaceRace: list = None, eventName: str = "", minGamesPerOpening: int = 0, pgnPathClock: str = None, playerColors: dict = None, relativeNumbers: bool = False, filename: str = None) -> None:
    if players is None:
    ...
    if pgnPathClock is not None:
        ...
        for player in clockLeads.keys():
            if nicknames and player in nicknames:
                ...
                clockLegend.append(player.split(',')[0])
    ...
    if filename:
        ...
        plotDecisiveGames(decisive, nicknames, title=f'Relative number of wins and losses{eventName}', relativeNumbers=relativeNumbers, filename=f'{filename}-decisive.png')
        ...
        plotDecisiveGames(decisive, nicknames, title=f'Relative number of wins and losses{eventName}', relativeNumbers=relativeNumbers)
...
def commandLine():
    ...
    parser.add_argument('--is-lichess-analysis', action='store_true', help='Set this option if the PGN file contains the analysis generated by Lichess')
    parser.add_argument('--event-name', default="", help='Optional. Name of the event used in the plot titles')
    parser.add_argument('--min-games-per-opening', default=0, help='The minimum number of games in an opening to be shown individually in the pie chart. All openings below the minimum get grouped as "other opening"', type=int)
    parser.add_argument('--clock-pgn-path', default=None, help='Path to the PGN file containing the clock times')
    parser.add_argument('--player-colors-path', default=None, help='Path to a YAML file containing the colors for each player')
    parser.add_argument('--relative-numbers', action='store_true', help='The relative number of games will be shown instead of the total number of games')
